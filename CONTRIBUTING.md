...
Thanks for your interest in contributing to Scribe of Lagash - Goals and Stats.
...
`<vault>/.obsidian/plugins/scribe-of-lagash-goals-stats/`, then enable the
...
at `src/data/<name>.ts` is `tests/data/<name>.test.ts` and imports its subject
from `../../src/data/<name>`. They cover the pure modules (title parsing, word
counting and tokenization, goal/pace calculations). `npm test` compiles them
with esbuild — already a build dependency — and runs them through Node's
built-in test runner; no separate test framework is pulled in.
...
- Note titles are classified by a pure module in `src/data/` with no Obsidian
  imports. Keep it that way — add new languages as extra entries in its pattern
  table, not as calls into the vault.
- Keep text analysis (tokenizing, word counting, stop-word filtering, pace and
  projection math) in pure modules under `src/data/` with no Obsidian imports,
  so it stays unit-testable.
- Never add code that modifies an existing chapter/scene note. This plugin only
  *reads* prose to measure it; it persists its own goal and stat data
  separately (in plugin data or its own files), never in the user's notes.
...
3. Bump the version — use the wrapper script for your bump size:
   ...
   `npm run version-patch -- -m "%s — hotfix for the word-count crash"`.
